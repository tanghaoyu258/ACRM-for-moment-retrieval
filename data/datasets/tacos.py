...
class TACOS(Dataset):
                    ...
                    anno_pairs.append(
                        {
                            'video': vid,
                            'fps': video_anno['fps'],
                            'number_frames':video_anno['num_frames'],
                            'duration': duration,
                            'times':[max(timestamp[0],0),min(timestamp[1],video_anno['num_frames'])],
    ...
    def __getitem__(self, index):
        ...
        fps = ann['fps']
        number_frames = ann['number_frames']
        visual_features = h5py.File(self.feature_path, 'r')
        ...
        word_idxs = torch.tensor([self.vocab.stoi.get(w.lower(), 1438) for w in sentence.split()], dtype=torch.long)
        tokens = self.word_embedding(word_idxs)

        feature_start = int(feat_length* time_start/float(number_frames))
        feature_end = min(math.ceil(feat_length*time_end/number_frames),feat_length-1)

        # feature_start = int(feat_length* time_start / duration)
        # feature_end = min(math.ceil(feat_length*time_end/duration),feat_length-1)
        
        factor = float(number_frames)/feat_length
        # factor = 16* self.sample_rate
        if False:
        ...
        return index, i3dfeat, tokens, torch.from_numpy(loc_start), torch.from_numpy(loc_end), torch.from_numpy(localization),\
                   time_start, time_end, ann['number_frames']/feat_length, fps, start,end, duration , video_id
